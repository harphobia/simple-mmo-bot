...
class Bot extends SimpleMMO {
    ...
    getDowngradeItems = (page) => new Promise((resolve, reject) => {
        fetch(`https://web.simple-mmo.com/inventory/items?order_col=items.stat1modifier&order=desc&page=${page}`, {
            method: 'GET',
            headers: {...header, 'Cookie': `XSRF-TOKEN=${this.xsrf_token}; laravelsession=${this.laravel_session}; ${this.remember_web_key}=${this.remember_web_value}; d_h=true`},
        })
        .then((res)=>res.text())
        .then((res) => {
            ...
            let itemList = []
            $('*tr[id^="item"]').each(async(i,el)=>{
                const downgrade = $(el).children('td:nth-child(6)').find('i[style="color:#c0392b;"]').length
                if(downgrade){
                    const itemId = $(el).attr('id').split('-').at(1)
    ...
    sellItemsToNpc = (itemList) => new Promise((resolve, reject) => {

        let params = ``
        for(let val of itemList){
            params += `&item_id%5B%5D=${val}`
        }

        fetch(`https://web.simple-mmo.com/api/multi-npc`, {
            method: 'POST',
            headers: {...header, 'Cookie': `XSRF-TOKEN=${this.xsrf_token}; laravelsession=${this.laravel_session}; ${this.remember_web_key}=${this.remember_web_value}; d_h=true`},
            body: `_token=${this.token}` + params
        })
        .then((res)=>res.json())
        .then((res) => resolve(res))
        .catch((err)=> reject(err))
